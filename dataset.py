...
class BilingualDataset(Dataset):
    def __init__(self, ds, src_tokenizer, tgt_toeknizer, src_lang, tgt_lang, seq_len) -> None:
        super().__init__()
        self.ds = ds
    ...
    def __getitem__(self, index):
        ...
        encoder_input = torch.cat(
            [
                ...
                self.eos_token,
                torch.tensor([self.pad_token] * enc_pad_tokens)
            ]
        )
        ...
        decoder_input = torch.cat([
            ...
            torch.tensor([self.pad_token] * dec_pad_tokens)
        ])
