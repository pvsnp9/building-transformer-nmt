...
class DecoderBlock(nn.Module):
    ...
    # ff_block
    def __init__(self, attention, cross_attention, ff_block, dropout) -> None:
        super().__init__()
    ...
    def forward(self, x, encoder_output, src_mask, target_maks):
        # first block in decoder targeted tokens and target masks
        x = self.residual_connection[0](x, lambda x: self.attention(x,x,x, target_maks))
